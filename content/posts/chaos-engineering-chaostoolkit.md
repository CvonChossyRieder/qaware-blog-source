...
The first part of the test describes the Steady State. The CTK core supports three different probe providers: HTTP, Process, Python[^1]. Our first example uses the HTTP provider and sends a HTTP request to the search service and vallidates its HTTP response code.
